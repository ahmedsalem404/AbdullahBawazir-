...
interface RegularReportModalProps {
  onClose: () => void;
  branch?: string;
}

/**
 * @component RegularReportModal
 * @description نافذة منبثقة لإدخال وعرض وطباعة تقرير الفحص العادي مع شروط وإخلاء المسؤولية المعتمدة.
 * يعتمد على الحالة المحلية (Stateless) دون الحفظ في قاعدة البيانات.
 */
export function RegularReportModal({ onClose, branch = "الحسوة" }: RegularReportModalProps) {
  ...
  async function sharePdf() {
      ...
      try {
        if (navigator.share && navigator.canShare && navigator.canShare({ files: [shareFile] })) {
          await navigator.share({
            files: [shareFile],
            title: "تقرير فحص عادي",
            text: `مرفق تقرير فحص عادي الخاص بالسيارة ${carType} من مركز عبدالله باوزير.`,
          });
          setShareFile(null);
        }
      } catch (err) {
    ...
    try {
      ...
      const filename = `Regular-Inspection-${carType || "car"}.pdf`;
  ...
  async function exportPdf() {
    ...
    try {
      ...
      pdf.addImage(image, "PNG", 0, 0, pageWidth, imageHeight);
      pdf.save(`Regular-Inspection-${carType || "car"}.pdf`);
    } catch (err) {
        ...
        <div className="max-h-[85vh] w-full overflow-y-auto rounded-[calc(2.5rem-0.5rem)] bg-white p-5 sm:p-8 border border-white/60 shadow-[inset_0_1px_1px_rgba(255,255,255,0.8)]">
          ...
          <div className="no-print space-y-6 mb-8">
            ...
            <div className="space-y-4">
              <div className="space-y-1.5">
                <label className="text-xs font-black text-slate-500">التشخيص بشكل عام</label>
                <textarea
                  value={diagnosis}
                  onChange={(e) => setDiagnosis(e.target.value)}
                  placeholder="أدخل الأعطال الحالية، وحالة الأجزاء المفحوصة بالتفصيل..."
                  rows={4}
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-black text-slate-500">توصيات العمل</label>
                <textarea
                  value={recommendations}
                  onChange={(e) => setRecommendations(e.target.value)}
                  placeholder="التوصيات الفنية اللازمة، القطع المطلوبة للاستبدال، والإصلاحات المقترحة..."
                  rows={4}
                  className="w-full rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm font-bold outline-none transition focus:border-indigo-400 focus:bg-white resize-y"
                />
              </div>
          ...
          >
            ...
            {/* General Diagnosis */}
            <div className="space-y-6 print:space-y-6 mb-8 print:mb-6">
              <section className="rounded-2xl border border-slate-200 p-6 print:p-5">
                ...
                <p className="min-h-24 print:min-h-24 print:text-sm whitespace-pre-wrap text-sm print:leading-relaxed leading-relaxed text-slate-700">
                  {diagnosis || "لا يوجد تشخيص مسجل"}
                </p>
              </section>

              <section className="rounded-2xl border border-slate-200 p-6 print:p-5">
                <h3 className="mb-3 print:mb-2 text-base sm:text-lg print:text-base font-bold text-slate-800 border-b border-slate-100 pb-2 print:pb-2">
                  🛠️ توصيات العمل والإصلاح المقترحة
                </h3>
                <p className="min-h-24 print:min-h-24 print:text-sm whitespace-pre-wrap text-sm print:leading-relaxed leading-relaxed text-slate-700">
                  {recommendations || "لا توجد توصيات مسجلة"}
                </p>
              </section>
            </div>

            {/* Disclaimer Section */}
            <div className="mb-8 print:mb-6 bg-red-50/50 p-5 print:p-5 rounded-2xl border border-red-100 text-sm print:text-xs leading-relaxed">
              <h4 className="font-black text-red-800 mb-2 print:mb-1 flex items-center gap-1.5">
                <span>⚠️ إخلاء مسؤولية فنية وقانونية:</span>
              </h4>
              <ul className="list-disc list-inside space-y-1 print:space-y-1 font-bold text-slate-600">
                <li>لا يمكننا تحديد كافة القطع التالفة بدقة إلا أثناء الفك وبدء العمل.</li>
                <li>المركز غير مسؤول عن أي أعطال خفية لم تظهر أثناء الفحص المبدئي.</li>
                <li>هذا التقرير هو تقييم مبدئي لحالة السيارة وقت الفحص فقط.</li>
              </ul>
            </div>
